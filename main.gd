...
func _end_round() -> void:
	...
	if _round_finds.is_empty():
		...
		for entry in _round_finds:
			names.append("%s  ·  %s  ·  %s" % [entry["name"], entry["grade"], entry.get("dirt", "")])
...
func _show_summary() -> void:
	summary.show_summary(GameState.money, _last_fossil_line, _last_fossil_stars)
...
func _on_fossil_extracted(fossil_name: String, value: int, integrity: float, cleanliness: float, clean: bool, piece_id: String) -> void:
	...
	var dirt := Tuning.dirt_label(cleanliness)
	_round_finds.append({"name": fossil_name, "grade": grade, "stars": stars, "dirt": dirt})
	toast.show_toast("%s found!" % fossil_name, "%s  ·  %s" % [grade, dirt], stars)
