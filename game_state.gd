...
var catalog: Array[Dictionary] = [
	...
	{"id": "hands_hold", "cat": "Hands", "tier": 1, "name": "Steady Hands", "desc": "Hold to keep digging. First rank unlocks it.", "cost": 32, "scale": 1.55, "max": 4},
	...
	{"id": "shovel_hold", "cat": "Shovel", "tier": 1, "name": "Steady Shoveling", "desc": "Holding the shovel digs faster.", "cost": 55, "scale": 1.65, "max": 5, "requires": "shovel_click"},
	{"id": "shovel_radius", "cat": "Shovel", "tier": 1, "name": "Wider Scoop", "desc": "The shovel covers more ground.", "cost": 90, "scale": 1.75, "max": 4, "requires": "shovel_click"},
	...
	{"id": "pick_hold", "cat": "Pickaxe", "tier": 1, "name": "Relentless Picking", "desc": "Holding the pickaxe strikes faster.", "cost": 160, "scale": 1.65, "max": 5, "requires": "pick_click"},
	...
	{"id": "precision", "cat": "Pickaxe", "tier": 2, "name": "Fine Point", "desc": "Unlock single-block mode. Later ranks hit harder in that mode.", "cost": 650, "scale": 1.8, "max": 5},
	...
	{"id": "scrap_bed", "cat": "Site", "tier": 1, "name": "Scattered Scraps", "desc": "A second small bone can hide in the pit.", "cost": 110, "scale": 1.75, "max": 2},
	{"id": "rich_bed", "cat": "Site", "tier": 2, "name": "Rich Bed", "desc": "Large bones can appear. Extra scraps too.", "cost": 480, "scale": 1.8, "max": 3},
	...
	{"id": "passive_miner", "cat": "Site", "tier": 3, "name": "Hired Hand", "desc": "A helper you can station on the claim before a shift. Placement comes later.", "cost": 3800, "scale": 1.0, "max": 1},
...
func tool_for_upgrade(id: String) -> int:
	if id.begins_with("hands") or id.begins_with("shovel"):
...
func owned_tool_ids() -> Array[int]:
	var tools: Array[int] = [Tuning.TOOL_SHOVEL]
...
func hold_unlocked() -> bool:
	return int(levels.get("hands_hold", 0)) > 0 or int(levels.get("shovel_hold", 0)) > 0
...
func shop_display_name(id: String) -> String:
	var item: Dictionary = _item(id)
	var level: int = int(levels.get(id, 0))
	if level <= 0 and item.has("unlock_name"):
...
func shop_item_desc(id: String) -> String:
	var item: Dictionary = _item(id)
	if int(levels.get(id, 0)) <= 0 and item.has("unlock_desc"):
...
func apply_upgrades() -> void:
	...
	Tuning.shovel_radius = 0.0 + 0.40 * _lv("shovel_radius") + 0.12 * _lv("shovel_super")
...
func piece_income(piece_id: String) -> float:
	...
	var hall: bool = stand_for_piece(piece_id) != ""
