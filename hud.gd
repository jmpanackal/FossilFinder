...
var _tool_flash: PackedFloat32Array = PackedFloat32Array([0.0, 0.0, 0.0])
...
func _ready() -> void:
	...
	_add_tool_slot(tools, Tuning.TOOL_SHOVEL, "1")
	_add_tool_slot(tools, Tuning.TOOL_PICKAXE, "2")
	_add_tool_slot(tools, Tuning.TOOL_BRUSH, "3")
	...
	_highlight_tool(Tuning.TOOL_SHOVEL)
...
func _add_tool_slot(parent: HBoxContainer, tool: int, hotkey: String) -> void:
...
func _highlight_tool(tool: int) -> void:
	...
	for i in _tool_buttons.size():
		var owned: bool = i == Tuning.TOOL_SHOVEL or GameState.owns_tool(i)
		var on: bool = owned and i == tool
		_tool_buttons[i].disabled = not owned
		Ui.apply_button(_tool_buttons[i], on)
		if not owned:
			_tool_buttons[i].modulate = Color(0.42, 0.4, 0.38)
		else:
			_tool_buttons[i].modulate = Color.WHITE if on else Color(0.78, 0.74, 0.68)


func _refresh_tool_icons() -> void:
	if _tool_icons.is_empty():
		return
	var shown: int = Tuning.TOOL_HANDS
	if GameState.owns_tool(Tuning.TOOL_SHOVEL):
		shown = Tuning.TOOL_SHOVEL
	var icon: Variant = _tool_icons[0]
	if icon != null and icon.has_method("setup"):
		icon.setup(shown, _tool_colors[0])


func _apply_tool_flashes() -> void:
	for i in _tool_buttons.size():
		if i >= _tool_flash.size() or _tool_flash[i] <= 0.0:
			continue
		var glow: float = _tool_flash[i]
