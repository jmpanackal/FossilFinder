...
func _ready() -> void:
	...
	_note.offset_left = 760
	_note.offset_right = -24
	_note.offset_top = 18
	_note.offset_bottom = 58
	...
	_pan = _clamp_pan(Vector2((_view().x - HALL.x) * 0.5, 0.0))
	_apply_pan()
...
func _exhibit_income_line() -> String:
	var line: String = "$%.2f / sec from the exhibit" % GameState.museum_income()
